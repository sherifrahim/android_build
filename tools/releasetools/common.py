...
class BlockDifference(object):
  ...
  def WriteScript(self, script, output_zip, progress=None,
    ...
    if not self.src:
      ...
      script.Print("Patching KangOS %s image unconditionally..." % (self.partition,))
    else:
      script.Print("Patching KangOS %s image after verification." % (self.partition,))
  ...
  def WritePostInstallVerifyScript(self, script):
    partition = self.partition
    script.Print('Verifying the updated KangOS %s image...' % (partition,))
    ...
    if self.tgt.extended:
      ...
      script.Print('Verified the updated KangOS %s image.' % (partition,))
      if partition == "system":
